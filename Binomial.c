...
int main(void) {
	...
	*/
	...
	time = 1;
	begin = clock();
	option_price = binomial('C', strike_price, time, stock_price, 0.2, 0.05, 0.04, 150000, 'E');
	end = clock();
	...
	*/
	int i;
	strike_price = 100;
	double early2[13] = {100.00, 91.30, 88.91, 87.35, 86.17, 85.23, 84.45, 83.77, 83.18, 82.66, 82.20, 81.77, 81.38};
	for (i = 0; i < 12; i++) {
		time = (double) (i+1)/12;
		begin = clock();
		option_price = binomial('P', strike_price, time, early2[i+1], 0.2, 0.05, 0, 1000, 'A');
		end = clock();
		double dif = option_price - (strike_price - early2[i+1]);
		printf("Price: %f\n", option_price);
		printf("S*(10): %f\n", early2[i+1]);
		printf("Difference: %f\n", dif);
		printf("Time: %f\n\n", (double)(end-begin) / CLOCKS_PER_SEC);
